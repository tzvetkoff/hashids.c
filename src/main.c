...
static void usage(const char *program_invocation_name, FILE *out) {
	...
	fprintf(out,
		"  -l, --min-length  set hash minimum length [%u]\n",
...
static unsigned long long parse_number(const char *s, char **p) {
	int radix = 10;
...
int main(int argc, char **argv) {
	hashids_t *hashids;
	char *salt = HASHIDS_DEFAULT_SALT, *alphabet = HASHIDS_DEFAULT_ALPHABET, *buffer, *p,
		 str[18];
	...
	/* no arguments? */
	if(optind == argc) { usage(argv[0], stderr); }
	...
	if(command == COMMAND_ENCODE) {
		...
		if(hex) {
			...
			buffer =
				(char *)calloc(hashids_estimate_encoded_size(hashids, 1, &number), 1);
		...
		numbers = (unsigned long long *)calloc(numbers_count, sizeof(unsigned long long));
		numbers_ptr = numbers;
		...
		buffer =
			(char *)calloc(hashids_estimate_encoded_size(hashids, numbers_count, numbers),
				1);
