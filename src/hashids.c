...
static inline unsigned short hashids_div_ceil_unsigned_short(unsigned short x,
	unsigned short															y) {
	return x / y + !!(x % y);
}

/* fast log2(x) for unsigned long long */
const unsigned short hashids_log2_64_tab[64] = {63,
	0,
	58,
	1,
	59,
...
static inline unsigned short hashids_log2_64(unsigned long long x) {
	...
	return hashids_log2_64_tab[((unsigned long long)((x - (x >> 1)) *
													 0x07EDD5E59A4E28C2)) >>
							   58];
}
...
void hashids_free(hashids_t *hashids) {
	if(hashids) {
		if(hashids->alphabet) { _hashids_free(hashids->alphabet); }
		if(hashids->alphabet_copy_1) { _hashids_free(hashids->alphabet_copy_1); }
		if(hashids->alphabet_copy_2) { _hashids_free(hashids->alphabet_copy_2); }
		if(hashids->salt) { _hashids_free(hashids->salt); }
		if(hashids->separators) { _hashids_free(hashids->separators); }
		if(hashids->guards) { _hashids_free(hashids->guards); }
...
/* common init */
hashids_t *hashids_init3(const char *salt, size_t min_hash_length, const char *alphabet) {
	hashids_t *result;
	size_t	   i, j, len;
	char	   ch, *p;
	...
	for(i = 0, j = 0; i < len; ++i) {
		ch = alphabet[i];
		if(!strchr(result->alphabet, ch)) { result->alphabet[j++] = ch; }
	}
	result->alphabet[j] = '\0';
	...
	j	= (size_t)(ceil((float)result->alphabet_length / HASHIDS_SEPARATOR_DIVISOR) + 1);
	if(j < len + 1) { j = len + 1; }
	...
	for(i = 0, j = 0; i < strlen(HASHIDS_DEFAULT_SEPARATORS); ++i) {
		...
		if((p = strchr(result->alphabet, ch))) {
			...
			memmove(p, p + 1, strlen(result->alphabet) - (p - result->alphabet));
		}
	}
	...
	if(result->separators_count) {
		hashids_shuffle(result->separators,
			result->separators_count,
			result->salt,
			result->salt_length);
	}
	...
	if(!result->separators_count ||
		...
		size_t separators_count =
			(size_t)ceil((float)result->alphabet_length / HASHIDS_SEPARATOR_DIVISOR);

		if(separators_count == 1) { separators_count = 2; }
		...
		if(separators_count > result->separators_count) {
			...
			memmove(result->alphabet,
				result->alphabet + diff,
	...
	/* shuffle alphabet */
	hashids_shuffle(result->alphabet,
	...
	/* allocate guards */
	result->guards_count =
		hashids_div_ceil_size_t(result->alphabet_length, HASHIDS_GUARD_DIVISOR);
	...
	if(HASHIDS_UNLIKELY(result->alphabet_length < 3)) {
		...
		memmove(result->separators,
			result->separators + result->guards_count,
		...
		memmove(result->alphabet,
			result->alphabet + result->guards_count,
	...
	result->alphabet_copy_1 = (char *)_hashids_alloc(result->alphabet_length + 1);
	result->alphabet_copy_2 = (char *)_hashids_alloc(result->alphabet_length + 1);
	if(HASHIDS_UNLIKELY(
		   !result->alphabet || !result->alphabet_copy_1 || !result->alphabet_copy_2)) {
		hashids_free(result);
		hashids_errno = HASHIDS_ERROR_ALLOC;
		return NULL;
	}
...
hashids_t *hashids_init2(const char *salt, size_t min_hash_length) {
	return hashids_init3(salt, min_hash_length, HASHIDS_DEFAULT_ALPHABET);
}

/* init with hash only */
hashids_t *hashids_init(const char *salt) {
	return hashids_init2(salt, HASHIDS_DEFAULT_MIN_HASH_LENGTH);
}

/* estimate buffer size (generic) */
size_t hashids_estimate_encoded_size(hashids_t *hashids, size_t numbers_count,
	...
	for(i = 0, result_len = 1; i < numbers_count; ++i) {
		if(numbers[i] == 0) {
			...
			result_len += hashids_div_ceil_unsigned_short(hashids_log2_64(numbers[i]),
							  hashids_log2_64(hashids->alphabet_length)) -
						  1;
		} else {
			result_len += hashids_div_ceil_unsigned_short(hashids_log2_64(numbers[i] + 1),
				hashids_log2_64(hashids->alphabet_length));
		}
	}

	if(numbers_count > 1) { result_len += numbers_count - 1; }

	if(result_len < hashids->min_hash_length) { result_len = hashids->min_hash_length; }

	return result_len + 2 /* fast log2 & ceil sometimes undershoot by 1 */;
}

/* estimate buffer size (variadic) */
size_t hashids_estimate_encoded_size_v(hashids_t *hashids, size_t numbers_count, ...) {
	size_t				i, result;
	unsigned long long *numbers;
	va_list				ap;

	numbers =
		(unsigned long long *)_hashids_alloc(numbers_count * sizeof(unsigned long long));
...
/* encode many (generic) */
size_t hashids_encode(hashids_t *hashids, char *buffer, size_t numbers_count,
	unsigned long long *numbers) {
	...
	strncpy(hashids->alphabet_copy_1, hashids->alphabet, hashids->alphabet_length);
	...
	strncat(hashids->alphabet_copy_2, hashids->salt, hashids->alphabet_length - 1);
	p	  = hashids->alphabet_copy_2 + hashids->salt_length + 1;
	p_max = hashids->alphabet_length - 1 - hashids->salt_length;
	if(p_max > 0) {
		strncat(hashids->alphabet_copy_2, hashids->alphabet, p_max);
	} else {
	...
	for(i = 0; i < numbers_count; ++i) {
		...
		if(p_max > 0) { strncpy(p, hashids->alphabet_copy_1, p_max); }

		/* shuffle the alphabet */
		hashids_shuffle(hashids->alphabet_copy_1,
			hashids->alphabet_length,
			hashids->alphabet_copy_2,
			hashids->alphabet_length);
	...
	if(result_len < hashids->min_hash_length) {
		...
		if(result_len < hashids->min_hash_length) {
			...
			half_length_ceil  = hashids_div_ceil_size_t(hashids->alphabet_length, 2);
			half_length_floor = floor((float)hashids->alphabet_length / 2);

			/* pad, pad, pad */
			while(result_len < hashids->min_hash_length) {
				/* shuffle the alphabet */
				strncpy(hashids->alphabet_copy_2,
					hashids->alphabet_copy_1,
					hashids->alphabet_length);
				hashids_shuffle(hashids->alphabet_copy_1,
					hashids->alphabet_length,
					hashids->alphabet_copy_2,
				...
				i = hashids_div_ceil_size_t(hashids->min_hash_length - result_len, 2);
				/* right pad from the beginning */
				j = floor((float)(hashids->min_hash_length - result_len) / 2);

				/* check bounds */
				if(i > half_length_ceil) { i = half_length_ceil; }
				if(j > half_length_floor) { j = half_length_floor; }
				...
				/* pad left */
				memmove(buffer,
					hashids->alphabet_copy_1 + hashids->alphabet_length - i,
					i);
				/* pad right */
...
/* encode one */
size_t hashids_encode_one(hashids_t *hashids, char *buffer, unsigned long long number) {
	return hashids_encode(hashids, buffer, 1, &number);
}

/* numbers count */
size_t hashids_numbers_count(hashids_t *hashids, const char *str) {
	...
	numbers_count = 0;
	while((ch = *str)) {
		if(strchr(hashids->guards, ch)) { break; }
...
/* decode */
size_t hashids_decode(hashids_t *hashids, const char *str, unsigned long long *numbers,
	size_t numbers_max) {
	size_t			   numbers_count;
	...
	if(!numbers || !numbers_max) { return hashids_numbers_count(hashids, str); }
	...
	while((ch = *str)) {
		...
		if(strchr(hashids->separators, ch)) {
			...
			*numbers++ = number;
			...
			/* check limit */
			if(++numbers_count >= numbers_max) { return numbers_count; }
...
/* unsafe decode */
size_t hashids_decode_unsafe(hashids_t *hashids, const char *str,
	unsigned long long *numbers) {
	return hashids_decode(hashids, str, numbers, (size_t)-1);
}

/* safe decode */
size_t hashids_decode_safe(hashids_t *hashids, const char *str,
